...
var video: Object = null ## Video class object of GDE GoZen which interacts with video files through FFmpeg.
...
func set_video_path(new_path: String) -> void:
	...
	if video == null:
		printerr("GDE GoZen native video backend is unavailable")
		return
	if debug:
		video.enable_debug()
	else:
...
func update_video(video_instance: Object, audio_stream: AudioStream = null) -> void:
	close()
	if !is_node_ready():
		await ready
	if !get_tree().root.is_node_ready():
...
func _update_video(new_video: Object) -> void:
	video = new_video
	if !is_open():
...
func _open_audio(stream_id: int = -1) -> void:
	var stream: Object = ClassDB.instantiate("AudioStreamFFmpeg")
	if stream == null:
		printerr("GDE GoZen native audio backend is unavailable")
		return

	var opened: Variant = stream.call("open", path, stream_id)
	var ok: bool = bool(opened) if typeof(opened) == TYPE_BOOL else int(opened) == OK
	if not ok:
	...
	if audio_stream == null:
		printerr("AudioStreamFFmpeg did not create a valid AudioStream")
		return
	audio_player.stream = audio_stream
