...
int IvoryRig360::add_bone(const Vector2 &head, const Vector2 &tail,
	...
	b.angle = 0.0;
	bone_.push_back(b);
	min_angle_.push_back((float)-Math_TAU);
	max_angle_.push_back((float)Math_TAU);
	Undo u;
...
void IvoryRig360::set_angle(int index, double radians) {
	...
	const double lo = index < (int)min_angle_.size() ? min_angle_[(size_t)index] : -Math_TAU;
	const double hi = index < (int)max_angle_.size() ? max_angle_[(size_t)index] : Math_TAU;
...
void IvoryRig360::set_angle_limits(int index, double min_radians, double max_radians) {
	...
	min_angle_[(size_t)index] = (float)clampd(min_radians, -Math_TAU, Math_TAU);
	max_angle_[(size_t)index] = (float)clampd(max_radians, -Math_TAU, Math_TAU);
...
Dictionary IvoryRig360::angle_limits(int index) const {
	...
	out["min"] = index < (int)min_angle_.size() ? min_angle_[(size_t)index] : -Math_TAU;
	out["max"] = index < (int)max_angle_.size() ? max_angle_[(size_t)index] : Math_TAU;
	return out;
}
...
void IvoryRig360::apply_actions() {
	...
	for (size_t i = 0; i < bone_.size(); ++i) {
		if (count[i] > 0) {
			...
			const double lo = min_angle_.empty() ? -Math_TAU : min_angle_[i];
			const double hi = max_angle_.empty() ? Math_TAU : max_angle_[i];
