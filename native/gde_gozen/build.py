...
ENABLED_MODULES = [
    ...
    # IVORY MP4 export: native LGPL-compatible MPEG-4 Part 2 + MP4 muxer.
...
def compile_ffmpeg_linux(arch: str) -> None:
    ...
    cmd = [
        ...
        "--enable-libvpx",
        "--enable-libtls",
        # MP4 export: H.264 through Android MediaCodec when available, plus native AAC.
        "--enable-jni",
        "--enable-mediacodec",
    ...
    # DISABLED_MODULES turns all muxers off for the small FFmpeg build; put
    # the one IVORY needs back after that switch so FFmpeg configure keeps it.
    cmd += ["--enable-encoder=mpeg4", "--enable-muxer=mp4", "--enable-protocol=file"]
...
def compile_ffmpeg_android(arch: str) -> None:
    ...
    cxx: str = f"{toolchain_bin}/{target_arch}{ANDROID_API_LEVEL}-clang++"
